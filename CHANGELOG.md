...
- `switch_child` does not GET another school's `/login` through the library session (that POSTs this account's password and can lock the child). Cross-school device verification uses a raw MFA POST only.
